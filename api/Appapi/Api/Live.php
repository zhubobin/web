			...
			for($i=0;$i<$nums;){
				...
				try{		 
					$result = $client->push()
							->setPlatform('all')
							->addAlias($alias)
							->setNotificationAlert('你的好友：'.$info['user_nicename'].'正在直播，邀请你一起')
							->addAndroidNotification('你的好友：'.$info['user_nicename'].'正在直播，邀请你一起', '', 1, array("userinfo"=>$info))
							->addIosNotification('你的好友：'.$info['user_nicename'].'正在直播，邀请你一起', 'iOS sound', JPush::DISABLE_BADGE, true, 'iOS category', array("userinfo"=>$info))
